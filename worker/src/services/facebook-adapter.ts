...
export function normalizeFacebookMessagingPayload(payload: unknown): NormalizedMessage[] {
    ...
    for (const event of messaging) {
      ...
      for (const rawAttachment of attachments) {
        ...
        if (senderId && type !== "text") {
          messages.push({
            body: `[${type} recebido via Facebook]`,
            channel: "facebook",
            direction: "inbound",
            externalConversationId: senderId,
            externalMessageId,
...
export async function sendFacebookText(env: Env, input: SendTextInput) {
...
export const facebookAdapter: MessageAdapter = {
  channel: "facebook",
  normalizeInbound: normalizeFacebookMessagingPayload,
